...
**Partially implemented: M1.1 and M1.2 complete under their separate user requests. M1.2's final independent review scores 9.12/10; M1.3–M1.4 remain deferred. Stop for user review.** This extends the reviewed simulated-movement proposal into the smallest complete operator workflow, before the retained Phase 5 compatibility work. It uses existing Phase 3B and brought-forward map functionality; completing all outstanding Phase 4 fidelity/coverage requirements is not a prerequisite for the explicitly limited model below.
...
**Delivery order:** Phase 3B review checkpoint (complete) → M1.1 authority/run entry (complete) → M1.2 movement workflow (complete) → authorized Fleet/Details and RTS refinements (complete; stop for user review) → M1.3 complete notional encounter workflow → M1.4 integration review → retained Phase 5 compatibility slice → remaining analytics, replay, workspace and hardening acceptance. Outstanding map work can proceed under separate scope where needed; do not treat this order as completion of Phase 4. This documentation update does not authorize the later phases.
...
**M1.1 implementation result (historical checkpoint):** the contract gate was resolved in [the contract decisions](m1.1/CONTRACT_DECISIONS.md). Actual long-pause lifecycle UI, immutable reconciliation, explicit control and restart/migration behavior were verified; future accepted-work/cancellation semantics used bounded test doubles. No movement or execution-Cancel UI was added by M1.1. That request completed only M1.1 and did not authorize M1.2.

**M1.2 implementation result:** the separate 15 September user request authorized movement. Its bounded coordinate, execution, cancellation, version and recovery decisions are recorded in [M1.2 contract decisions](m1.2/CONTRACT_DECISIONS.md). Real execution now verifies the applicable accepted-work policies; later encounter and integrated-workload prerequisites remain outstanding. Stop after M1.2 for user review.
