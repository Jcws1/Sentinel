...
**Active roadmap — 18 September 2026:** the unimplemented M1.3/M1.4 roadmap is replaced by **D1–D7: Authored demo behavior and UI** in [the detailed proposal](DEMO_BEHAVIOR_UI_PLAN.md). Subsequent user requests authorize **D1a, bounded D1b and D2**. D2 is implemented and its final acceptance gate has passed; D3–D7 remain proposed. Preserve completed M1.1/M1.2, Fleet/Details, RTS and compact-demo work. The dated checkpoint notes below describe their original deliveries, not current restrictions on the new proposal. The former operator-declared-only encounter model and prohibitions on bounded scripting, automatic approach and coordination are explicitly superseded for the proposed local demo; those behaviors are not implemented by D1a, D1b or D2. Authority, persistence, recording, recovery and regression obligations remain. The [unchanged pre-revision plan](IMPLEMENTATION_PLAN.pre-demo-roadmap-2026-09-17.md) preserves the full historical roadmap.
...
**Current slice:** D1a, bounded D1b and D2 are implemented and reviewed. D2's final gate passes at 9.08/10; stop for user review. **D3–D7 — Authored demo behavior and UI** require subsequent implementation authorization. Those later phases add timed movements, bounded patrol/intercept policies, persistent simulated outcomes, reserves, a simulated cockpit and rules-based decision cards. An actual LLM, advanced swarm behavior and physical vehicle/sensor/effects models remain later work. See [the complete phases](DEMO_BEHAVIOR_UI_PLAN.md).
...
The interface sketch below expresses the boundary, not the exact implemented renderer API. Group/destination picking and requested-target projection are implemented; D2 adds typed boundary/edit projections; D4 condition, assignment and outcome projections remain proposed; no command execution or canonical state enters either renderer.
...
**D1a, bounded D1b and D2 are implemented and reviewed; D3–D7 remain proposed only.** The previous M1.3 and M1.4 are retired as pending milestones. Use the [detailed plan](DEMO_BEHAVIOR_UI_PLAN.md) for operator workflows, concrete existing/proposed module paths, contracts/migrations, bounded defaults, dependencies, risks, acceptance and deferrals. The [D1a review](d1a/REVIEW.md), [D1b review](d1b/REVIEW.md) and [D2 review](d2/REVIEW.md) record their separate delivery gates.
...
| D3 — Conductor and deterministic scenario execution | Timed friendly/hostile motion, simulation-time scheduling and visible per-friendly manual override | D1–D2; same writer, lifecycle, recording and command owner. |
| D4 — Fleet policies, patrol, coordinated interception and persistent outcomes | Compact Fleet Apply/Stop, deterministic unique targets/reserves, toy approach/engagement, persistent NON-OP | Replaces M1.3; retains its atomic outcome, cancellation, recovery and presentation obligations. |
...
**Behavior decisions:** scenarios and boundaries are frozen during a run; restrictions apply to scenario-owned drones, including scripted hostiles; Patrol requires explicit assignment; live friendly commands override future script until explicit Return to script; one interceptor per target with excess held in reserve; Stop disarms and prevents automatic resumption. These recommended defaults and material alternatives are explicit in the new plan. Affiliation never grants command authority. All behavior remains local simulation without validated aircraft, sensor, navigation or effects claims.

**Current implementation boundary:** verified D1a + bounded D1b + D2. Exact supported poses, explicit roles, immutable revisions/recordings, recovery and existing control remain, with boundary authoring and authoritative restrictions added. [D2 evidence](d2/REVIEW.md) records its passing third independent gate and two minor limitations; D1 delivery records remain historical. Expanded catalogs, teams, import/export, live definition editing and all D3–D7 behaviors remain deferred. Stop after D2 for review; its foundation is ready for D3 handoff under separate authorization.
...
Historical proposal/implementation review records remain at their original paths, and the former M1 plan remains in the unchanged archive. D1a, D1b and D2 have separate implementation decisions, test results, screenshots and independent review rounds under `docs/d1a/`, `docs/d1b/` and `docs/d2/`; prior scores are not reused. Stop after D2 for user review.
