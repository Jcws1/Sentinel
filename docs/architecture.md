...
The documented baseline is the cleaned **working tree**, not just HEAD `30414540b89508193ea84c4f8e235362ebd61a37`. All 788 initial files matched the cleanup's final inventory. The [documentation ledger](maintenance/documentation-2026-09-21.md) records final document/source hashes, command checks and independent review. No application behavior changes accompany this document.
...
[validation](../backend/app/simulation/validation.py) handles strict external JSON/schema/numerical validation; [resolver](../backend/app/simulation/resolver.py) remains a pure resolution boundary. [service](../backend/app/simulation/service.py) yields cooperatively during validation/resolution, but the final full-batch commit is synchronous and can block other source work.
...
Cold observed-history validation remains expensive: a retained 451-observation probe took 4,528.6 ms cold versus 107.0/94.7/92.6 ms warm, with identical returned data. This is a backend diagnostic on its named candidate, not a foreground acceptance pass. The repository cache preserves strict cold validation. Phase 5 separately records multi-second large-batch source blocking. D7 storage compression materially reduced retained bytes in its matched soak, but recording still grows; strict display/configured Video remain incomplete. Follow [D7 measurements](d7-details-closure/PERFORMANCE.md), [pacing](d7-details-closure/PACING.md) and [Phase 5 measurements](phase5-simulation-compatibility/PERFORMANCE.md) for their own baselines.
...
| Typed external boundary — implemented, conformance partial | Preserve frozen external semantics and provenance without granting generic interactive authority. Separate run/control UI adds complexity. | simulation + adapter; [Phase 5 decisions](phase5-simulation-compatibility/DECISIONS.md). |
...
| P5-GEOMETRY / material, inherited | domain/models.py Polygon validation; frontend contracts/integrity.ts; simulation/validation.py accepts external decimal geometry | Valid mixed-scale polygon returns HTTP500 in retained Phase5 reproduction | Valid external input can fail when mapped to the generic world | Keep compatibility limitation visible; reconcile valid geometry handling without lowering limits or changing frozen inputs | **Open**; original reproduction and negative/golden contracts must pass independently | [Phase5 critic R3-1](phase5-simulation-compatibility/critic-round-3.md), [verification](phase5-simulation-compatibility/VERIFICATION.md) |
| P5-BATCH / material, inherited | SimulationService._complete and repository commit | Large external batches produce measured multi-second source blocking | Interactive source responsiveness degrades despite separate mission ownership | Bound operational batch use; future work must preserve durable atomic completion and supported limits | **Open**; matched concurrent-source measurement needed | [Phase5 performance](phase5-simulation-compatibility/PERFORMANCE.md), test_simulation_service |
| P5-CONFORMANCE / unresolved contract | external validation/policy/resolver | Organizer/error/lifecycle/canonicalization/correction interpretations are provisional | Passing local goldens cannot certify external organizer compatibility | Seek explicit contract decisions; retain original request and fixtures | **Open**; resolve or explicitly accept each register item | [compatibility register](../contracts/simulation/compatibility-decisions.md) |
