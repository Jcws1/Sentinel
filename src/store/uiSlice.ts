...
interface UiState {
  ...
  /** Full telemetry card ΓÇö long-press fleet card to open. */
  telemetryExpanded: boolean
  /** Optional alert chirp on new threat alerts (off by default). */
  alertChirpEnabled: boolean
  helpOpen: boolean
  /** Pause auto-focus camera/selection churn during active review. */
  autoFocusFrozen: boolean
  /** Double-tap safety for multi-track confirm. */
...
const uiSlice = createSlice({
  ...
  reducers: {
    ...
    toggleAutoFocusFrozen(state) {
      state.autoFocusFrozen = !state.autoFocusFrozen
    },
    setAutoFocusFrozen(state, action: PayloadAction<boolean>) {
      state.autoFocusFrozen = action.payload
    },
    armConfirm(state) {
      state.confirmArmed = true
      state.confirmArmExpiresAt = Date.now() + 5000
    },
    disarmConfirm(state) {
      state.confirmArmed = false
      state.confirmArmExpiresAt = null
    },
