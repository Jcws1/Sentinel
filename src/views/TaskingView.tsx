...
export function TaskingView() {
  ...
  const confirmAssistantTask = () => {
    if (!assistantArmed) { setAssistantArmed(true); window.setTimeout(() => setAssistantArmed(false), 5000); return }
    setTaskStatus(selected.id, 'executing')
    addEvent(`Assistant tasking confirmed for ${selected.assetIds.join(' / ')}`, selected.id, 'info', 'Operator')
  ...
  return <div className="flex flex-col gap-4 p-3">
    ...
    {selected.source === 'assistant' && <Section title="Assistant recommendation"><div className="rounded-sm border border-signal-nominal/40 bg-state-selected p-2"><div className="flex items-start justify-between gap-2"><strong className="text-xs text-text">{selected.objective}</strong><span className="font-mono text-sm text-signal-nominal">{selected.allocationPct}%</span></div><p className="mt-1 text-2xs text-text-secondary">TRACK · {selected.trackId}</p><div className="mt-2 flex flex-wrap gap-1">{selected.assetIds.map((id) => <span key={id} className="rounded-xs border border-signal-nominal/30 px-1 py-0.5 font-mono text-2xs text-text">{id}</span>)}</div><button disabled={selected.status === 'executing'} onClick={confirmAssistantTask} className="mt-2 w-full rounded-sm border border-signal-nominal/60 p-2 text-xs text-signal-nominal disabled:text-text-disabled">{selected.status === 'executing' ? 'TASKING ACTIVE' : assistantArmed ? `CONFIRM / ${selected.assetIds.length} DRONES` : `TASK ACCORDINGLY / ${selected.assetIds.length} DRONES`}</button>{assistantArmed && <p className="mt-1 text-center text-2xs text-signal-caution">Press again within five seconds to start the training task.</p>}</div></Section>}
