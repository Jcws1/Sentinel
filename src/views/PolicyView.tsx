...
export function PolicyView() {
  ...
  const [audit, setAudit] = useState(['v12 · Intercept changed to review required · Mission Commander', 'v11 · Protected buffer changed to 500 m · Operations Supervisor'])
  ...
  return <div className="flex flex-col gap-4 p-3">
    <div className="rounded-sm border border-signal-nominal/30 bg-panel-inset p-2"><div className="flex justify-between"><strong className="text-xs text-signal-nominal">ACTIVE CONFIGURATION</strong><span className="font-mono text-2xs">v{version}</span></div><div className="mt-2 flex gap-1"><select aria-label="Policy environment" className={`${control} flex-1`} value={environment} onChange={(e) => setEnvironment(e.target.value)}><option>Operational</option><option>Training</option><option>Development</option></select>{editing ? <><button className={control} onClick={() => { setEditing(false); setDirty(false) }}>CANCEL</button><button className={`${control} ${dirty ? 'text-signal-nominal' : 'text-text-disabled'}`} disabled={!dirty} onClick={publish}>PUBLISH</button></> : <button className={control} onClick={() => setEditing(true)}>EDIT</button>}</div>{editing && <p className="mt-2 text-2xs text-signal-caution">Editing draft. Active policy is unchanged until publish.</p>}</div>
    ...
    {section === 'oversight' && <Section title="Human oversight"><div className="flex flex-col gap-1 rounded-sm border border-border-faint bg-panel-inset p-2"><StatusRow label="Protected locations" value="REVIEW"/><StatusRow label="Autonomous changes" value="REVIEW"/><StatusRow label="Intercept actions" value="REVIEW"/><StatusRow label="Primary reviewer" value="MISSION COMMANDER"/><StatusRow label="Fallback" value="OPS SUPERVISOR"/><StatusRow label="Timeout" value="2 min → HOLD"/></div></Section>}
    ...
    <Section title="Dry-run live state"><div className="flex gap-1"><select aria-label="Dry-run track" className={`${control} flex-1`} value={dryRun} onChange={(e) => setDryRun(e.target.value)}><option>T-04</option><option>T-07</option><option>T-03</option></select><span className={`rounded-sm border px-2 py-1 text-2xs ${dryRun === 'T-03' ? 'border-signal-caution/40 text-signal-caution' : 'border-signal-nominal/40 text-signal-nominal'}`}>{dryRun === 'T-03' ? 'APPROVAL' : 'ELIGIBLE'}</span></div><p className="mt-1 text-2xs text-text-tertiary">Local rehearsal result · policy version v{version}</p></Section>
  </div>
}
