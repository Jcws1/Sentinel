...
export function FleetView() {
  ...
  return <div className="flex flex-col gap-4 p-3">
    <Section title="Fleet status"><div className="grid grid-cols-4 gap-1 text-center">{[
    ...
    <Section title="Fleet controls"><div className="grid grid-cols-2 gap-1.5"><button type="button" onClick={() => setMissionState(ops.missionState === 'HOLD' ? 'ACTIVE' : 'HOLD')} className="rounded-sm border border-signal-caution/40 px-2 py-2 text-xs text-signal-caution">{ops.missionState === 'HOLD' ? 'RESUME' : 'PAUSE'}</button><button type="button" onClick={() => setMissionState('RECALL')} className="rounded-sm border border-signal-critical/40 px-2 py-2 text-xs text-signal-critical">RETURN</button></div></Section>
  </div>
}
