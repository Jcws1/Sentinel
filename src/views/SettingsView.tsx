...
export function SettingsView() {
  ...
  return <div className="flex flex-col gap-4 p-3">
    ...
    <Section title="Adapters"><div className="flex flex-col gap-1.5"><div className="flex justify-between rounded-sm border border-border-faint bg-panel-inset p-2 text-2xs"><span>LOCAL DRONES</span><span className="text-signal-nominal">READY</span></div><div className="rounded-sm border border-border-faint bg-panel-inset p-2 text-2xs"><div className="flex justify-between"><span>WEDGETAIL TRAINING</span><span className={ops.wedgetail.status === 'error' ? 'text-signal-critical' : 'text-signal-nominal'}>{ops.wedgetail.status.toUpperCase()}</span></div><p className="mt-1 text-text-tertiary">Training-only adapter · no real systems connected</p></div></div></Section>
  </div>
}
