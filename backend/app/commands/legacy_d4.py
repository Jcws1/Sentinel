...
from app.commands.legacy_d4_behavior import LegacyD4BehaviorPolicy as BehaviorPolicy, LegacyD4BehaviorMemberOutcome as BehaviorMemberOutcome
...
class LegacyD4MoveAnchor(Model):
    longitude_deg: Longitude
    latitude_deg: Latitude


class LegacyD4MoveAltitude(Model):
    metres: Finite
    reference: Literal["ELLIPSOID"] = "ELLIPSOID"
    datum_id: Literal["WGS84"] = "WGS84"


class LegacyD4MovePosition(LegacyD4MoveAnchor):
    altitude: LegacyD4MoveAltitude


class LegacyD4MoveMember(Model):
    asset_id: Id
    entity_id: Id
    executor_id: Id
    source_id: Id
...
class LegacyD4MoveIntent(Model):
    ...
    deadline: UtcInstant
    anchor: LegacyD4MoveAnchor
    members: list[LegacyD4MoveMember] = Field(min_length=1, max_length=32)


class LegacyD4MoveRequest(Model):
    command_id: Id
    holder_id: Id
    move: LegacyD4MoveIntent
    order: int | None = Field(default=None, ge=1, le=9007199254740991, strict=True)


class LegacyD4DirectMoveMember(Model):
    asset_id: Id
    entity_id: Id
    executor_id: Id
    source_id: Id
...
class LegacyD4DirectMoveIntent(Model):
    ...
    anchor: LegacyD4MoveAnchor
    members: list[LegacyD4DirectMoveMember] = Field(min_length=1, max_length=32)


class LegacyD4DirectMoveRequest(Model):
    command_id: Id
    holder_id: Id
    direct: LegacyD4DirectMoveIntent


class LegacyD4DirectOrderContext(Model):
    holder_id: Id
    executor_epoch: Id
    grant_id: Id
    grant_revision: Sequence
...
class LegacyD4DirectMemberOutcome(Model):
    asset_id: Id
    entity_id: Id
...
class LegacyD4ControlMemberOutcome(Model):
    asset_id: Id
    entity_id: Id
    outcome: Literal["accepted", "skipped"]
...
class LegacyD4CompletionSample(Model):
    sequence: Sequence
    track_id: Id
...
class LegacyD4MovementExecution(LegacyD4MoveMember):
    ...
    completion_sample: LegacyD4CompletionSample | None = None
...
class LegacyD4AssetControl(Model):
    mission_id: Id
    asset_id: Id
    entity_id: Id
    executor_id: Id
    ...
    eligible: bool = False
    reason: str
    last_direct_order: LegacyD4DirectOrderContext | None = None


class LegacyD4InteractiveRun(Model):
    ...
    controls: list[LegacyD4AssetControl] = Field(max_length=32)
    movement_model: Literal["local-horizontal-v1"] = "local-horizontal-v1"
    executions: list[LegacyD4MovementExecution] = Field(default_factory=list, max_length=64)
...
class LegacyD4Intent(Model):
    ...
    members: list[LegacyD4DirectMoveMember] | None = Field(default=None, min_length=1, max_length=32)
    order: int | None = Field(default=None, ge=1, le=9007199254740991, strict=True)
...
class LegacyD4IntentRequest(Model):
    policy: BehaviorPolicy | None = None
    action: Action
    boundary: BoundaryMutation | None = None
    execution_id: Id | None = None
...
class LegacyD4CommandRequest(Model):
    command_id: Id
    holder_id: Id
    intent: LegacyD4Intent


class LegacyD4CreateRunRequest(Model):
    creation_id: Id
...
class LegacyD4Receipt(Model):
    ...
    direct_order: int | None = Field(default=None, ge=1, le=9007199254740991, strict=True)
    member_outcomes: list[LegacyD4DirectMemberOutcome] = Field(default_factory=list, max_length=32)
    control_order: int | None = Field(default=None, ge=1, le=9007199254740991, strict=True)
    control_outcomes: list[LegacyD4ControlMemberOutcome] = Field(default_factory=list, max_length=32)
...
ReceiptRead = LegacyD4Receipt | LegacyD3aReceipt | LegacyD3Receipt | LegacyD2Receipt | LegacyM12Receipt | LegacyReceipt


class LegacyD4ExecutionRead(Model):
    schema_version: Literal["1.2"] = "1.2"
    mission_id: Id
    frame_id: Id
    sequence: Sequence
    executions: list[LegacyD4MovementExecution] = Field(max_length=64)


class LegacyD4DemoEntry(Model):
...
class LegacyD4CommandContracts(Model):
    """Export root; no route accepts this aggregate."""
    intent: LegacyD4Intent
    command: LegacyD4CommandRequest
    creation: LegacyD4CreateRunRequest
    receipt: ReceiptRead
    status: LegacyD4RunRead
    legacy_d3a_status: LegacyD3aRunRead
    entry: LegacyD4DemoEntry
    move: LegacyD4MoveRequest
    direct_move: LegacyD4DirectMoveRequest
    executions: LegacyD4ExecutionRead
