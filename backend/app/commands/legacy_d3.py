"""Frozen D3 models: strict historical reading only."""
...
LegacyD3Action = Literal["acquire", "renew", "reclaim", "revoke", "start", "pause", "resume", "end", "cancel", "stop", "return-to-script"]
LegacyD3RunState = Literal["ready", "running", "paused", "ended"]
LegacyD3Code = Literal["OK", "NOT_INTERACTIVE", "DEMO_DISABLED", "ACTIVE_RUN_EXISTS", "IDENTITY_CONFLICT",
...
class LegacyD3MoveAnchor(Model):
    longitude_deg: Longitude
    latitude_deg: Latitude


class LegacyD3MoveAltitude(Model):
    metres: Finite
    reference: Literal["ELLIPSOID"] = "ELLIPSOID"
    datum_id: Literal["WGS84"] = "WGS84"


class LegacyD3MovePosition(LegacyD3MoveAnchor):
    altitude: LegacyD3MoveAltitude


class LegacyD3MoveMember(Model):
    asset_id: Id
    entity_id: Id
    executor_id: Id
    source_id: Id
    ...
    origin: LegacyD3MovePosition
    destination: LegacyD3MovePosition


class LegacyD3MoveIntent(Model):
    ...
    deadline: UtcInstant
    anchor: LegacyD3MoveAnchor
    members: list[LegacyD3MoveMember] = Field(min_length=1, max_length=32)


class LegacyD3MoveRequest(Model):
    command_id: Id
    holder_id: Id
    move: LegacyD3MoveIntent


class LegacyD3DirectMoveMember(Model):
    asset_id: Id
    entity_id: Id
    executor_id: Id
    source_id: Id
...
class LegacyD3DirectMoveIntent(Model):
    model_id: Literal["local-horizontal-v1"] = "local-horizontal-v1"
    mission_id: Id
    run_id: Id
    executor_epoch: Id
    ...
    anchor: LegacyD3MoveAnchor
    members: list[LegacyD3DirectMoveMember] = Field(min_length=1, max_length=32)


class LegacyD3DirectMoveRequest(Model):
    command_id: Id
    holder_id: Id
    direct: LegacyD3DirectMoveIntent


class LegacyD3DirectOrderContext(Model):
    holder_id: Id
    executor_epoch: Id
    grant_id: Id
    grant_revision: Sequence
...
class LegacyD3DirectMemberOutcome(Model):
    asset_id: Id
    entity_id: Id
...
class LegacyD3ControlMemberOutcome(Model):
    asset_id: Id
    entity_id: Id
    outcome: Literal["accepted", "skipped"]
...
class LegacyD3CompletionSample(Model):
    sequence: Sequence
    track_id: Id
    timestamp: UtcInstant
    position: LegacyD3MovePosition


class LegacyD3MovementExecution(LegacyD3MoveMember):
    ...
    completion_sample: LegacyD3CompletionSample | None = None
...
class LegacyD3AssetControl(Model):
    mission_id: Id
    asset_id: Id
    entity_id: Id
    executor_id: Id
    ...
    eligible: bool = False
    reason: str
    last_direct_order: LegacyD3DirectOrderContext | None = None


class LegacyD3InteractiveRun(Model):
    ...
    executor_epoch: Id
    source_id: Id
    state: LegacyD3RunState
    run_revision: Sequence
    grant_id: Id
    grant_revision: Sequence
    capabilities: list[Literal["run-control", "scenario-pair"]]
    supported_actions: list[LegacyD3Action]
    lease: LegacyD3Lease
    tick: Sequence
    last_report_at: UtcInstant | None = None
    controls: list[LegacyD3AssetControl] = Field(max_length=32)
    movement_model: Literal["local-horizontal-v1"] = "local-horizontal-v1"
    executions: list[LegacyD3MovementExecution] = Field(default_factory=list, max_length=64)
...
class LegacyD3Intent(Model):
    ...
    action: LegacyD3Action
    issued_at: UtcInstant
    expires_at: UtcInstant
    execution_id: Id | None = None
    execution_revision: Sequence | None = None
    members: list[LegacyD3DirectMoveMember] | None = Field(default=None, min_length=1, max_length=32)
...
class LegacyD3IntentRequest(Model):
    action: LegacyD3Action
    execution_id: Id | None = None
    members: list[LegacyD3DirectMoveMember] | None = Field(default=None, min_length=1, max_length=32)
    order: int | None = Field(default=None, ge=1, le=9007199254740991, strict=True)
...
class LegacyD3CommandRequest(Model):
    command_id: Id
    holder_id: Id
    intent: LegacyD3Intent


class LegacyD3CreateRunRequest(Model):
    creation_id: Id
...
class LegacyD3Receipt(Model):
    schema_version: Literal["1.3"] = "1.3"
    request_id: Id
    operation: Literal["create", "acquire", "renew", "reclaim", "revoke", "start", "pause", "resume", "end", "move", "cancel", "direct-move", "stop", "return-to-script"]
    accepted: bool
    code: LegacyD3Code
    message: str
    ...
    direct_order: int | None = Field(default=None, ge=1, le=9007199254740991, strict=True)
    member_outcomes: list[LegacyD3DirectMemberOutcome] = Field(default_factory=list, max_length=32)
    control_order: int | None = Field(default=None, ge=1, le=9007199254740991, strict=True)
    control_outcomes: list[LegacyD3ControlMemberOutcome] = Field(default_factory=list, max_length=32)
...
class LegacyD3RunRead(Model):
    schema_version: Literal["1.4"] = "1.4"
    server_time: UtcInstant
    frame_id: Id
    sequence: Sequence
    run: LegacyD3InteractiveRun
    owns_control: bool
    lease_state: Literal["unclaimed", "held", "expired"]


LegacyD3ReceiptRead = LegacyD3Receipt | LegacyD2Receipt | LegacyM12Receipt | LegacyReceipt


class LegacyD3ExecutionRead(Model):
    schema_version: Literal["1.2"] = "1.2"
    mission_id: Id
    frame_id: Id
    sequence: Sequence
    executions: list[LegacyD3MovementExecution] = Field(max_length=64)


class LegacyD3DemoEntry(Model):
...
class LegacyD3CommandContracts(Model):
    """Export root; no route accepts this aggregate."""
    intent: LegacyD3Intent
    command: LegacyD3CommandRequest
    creation: LegacyD3CreateRunRequest
    receipt: LegacyD3ReceiptRead
    status: LegacyD3RunRead
    entry: LegacyD3DemoEntry
    move: LegacyD3MoveRequest
    direct_move: LegacyD3DirectMoveRequest
    executions: LegacyD3ExecutionRead
