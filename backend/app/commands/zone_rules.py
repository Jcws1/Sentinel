...
def restricted(frame):
    ...
    if not rules:
        return []
    return [(frame["zones"][key]["label"], [point(v) for v in frame["zones"][key]["geometry"]["coordinates"][0][:-1]])
...
def scenario_issues(content):
    ...
    for boundary in content.boundaries or []:
        ...
        if boundary.type == "restricted":
            ring = [point(v) for v in boundary.vertices]
            for unit in content.units:
                if contains(metric(unit.position.model_dump(by_alias=True)), ring):
