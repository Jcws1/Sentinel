...
class LegacyD4BehaviorAltitude(Model):
    metres: Finite
    reference: Literal["ELLIPSOID"] = "ELLIPSOID"
    datum_id: Literal["WGS84"] = "WGS84"


class LegacyD4BehaviorPosition(Model):
    longitude_deg: Longitude
    latitude_deg: Latitude
    altitude: LegacyD4BehaviorAltitude


class LegacyD4BehaviorPolicy(Model):
...
class LegacyD4EngagementModel(Model):
    ...
    acquisition_radius_m: Literal[250.0] = 250.0
...
class LegacyD4BehaviorMemberOutcome(Model):
    asset_id: Id
    entity_id: Id
...
class LegacyD4PatrolRoute(Model):
    boundary_id: Id
    geometry_hash: str = Field(pattern=r"^[a-f0-9]{64}$")
    loop: list[LegacyD4BehaviorPosition] = Field(min_length=3, max_length=32)
...
class LegacyD4BehaviorState(Model):
    id: Id
    command_id: Id
    asset_id: Id
    entity_id: Id
    ...
    patrol: LegacyD4PatrolRoute | None = None
    ...
    def state_evidence(self):
        ...
        if self.state in {"patrolling", "pursuing"} and (self.control_track_id is None or self.deadline is None):
...
class LegacyD4InterceptAssignment(Model):
    id: Id
    policy_id: Id
    command_id: Id
    asset_id: Id
...
class LegacyD4EngagementParticipant(Model):
    entity_id: Id
    track_id: Id
    affiliation: Literal["friendly", "hostile"]
    before: LegacyD4BehaviorPosition
    proposed: LegacyD4BehaviorPosition
    evaluated: LegacyD4BehaviorPosition
    before_condition: Literal["operational"] = "operational"
    after_condition: Literal["non-operational"] = "non-operational"


class LegacyD4DemoOutcome(Model):
    id: Id
    rule_version: Literal["demo-mutual-loss-v1"] = "demo-mutual-loss-v1"
    assignment_id: Id
    policy_id: Id
    ...
    participants: list[LegacyD4EngagementParticipant] = Field(min_length=2, max_length=2)


class LegacyD4FleetBehavior(Model):
    rule_version: Literal["local-fleet-v1"] = "local-fleet-v1"
    run_id: Id
    source_id: Id
    model: LegacyD4EngagementModel
    members: list[LegacyD4BehaviorState] = Field(default_factory=list, max_length=32)
    assignments: list[LegacyD4InterceptAssignment] = Field(default_factory=list, max_length=64)
    outcomes: list[LegacyD4DemoOutcome] = Field(default_factory=list, max_length=32)
