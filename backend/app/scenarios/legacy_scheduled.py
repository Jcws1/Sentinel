...
class ScenarioContent(Model):
    ...
    schedule_rule_version: Literal["local-schedule-v1"] | None = None
    ...
    def identities(self):
        ...
        actor_ticks = [(a.unit_id, a.offset_ms) for a in actions]
...
def content_version(content):
    return "1.2" if content.actions is not None else "1.1" if content.boundaries is not None else "1.0"
