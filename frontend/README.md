# RTS maps and demo operation

Start the backend with `SENTINEL_DEMO=1` (optionally `SENTINEL_FIXTURES=1`) and run `npm run dev`. Choose **New demo**: the shared runtime creates a fresh recording, acquires available control and starts. Existing active demos are reopened through Simulation. Another session's valid control is never displaced; an expired conflicting owner requires explicit Take control. Pause/Resume is contextual, End is secondary and preserves the recording. Demo numbers persist across reloads and restarts.

Select drones from Fleet, Tracks or either map, then right-click a destination to move. A later valid click replaces movement for the selected available members from committed positions; other members continue. Unavailable members remain selected with explicit skipped outcomes, and invalid replacements preserve prior work. Both Pan and Select permit click selection. Pan drags the camera; Select draws a rectangle around visible managed friendlies, including unavailable members. Shift toggles/adds, Space temporarily pans, Escape cancels an active gesture or clears selection. Right-drag rotates/orbits without commanding. The Move button arms a destination click; focused-map Enter supplies a keyboard path.

3D destinations use the displayed surface's longitude/latitude, including roofs, and retain each drone's supplied flight height/reference. Loading/absent geometry and sky reject with feedback. No landing, terrain clearance or planned route is implied. Recenter returns to the local Singapore area; Map layers offers Overview and Focus selection. Close views use finer cursor-centred zoom, independent camera bookmarks and the existing providers/data. No browser interpolation changes the authoritative 5 Hz movement samples.

## Fleet, Details and Activity

The 220 px left sidebar switches between Views/Fleet. Details and Activity share a resizable right stack, normally 340 px (300–440 px). Below 1180 px the left sidebar collapses first; below 900 px the auxiliary stack docks below the map (220–320 px). Each map retains its camera and renderer lease. Fleet membership does not filter opposing or unmanaged map observations.

Explicit selection opens one selection-following Details pane; live frames never reopen it after closure. Pin inspector remains a separate fixed mission/entity surface. Details presents short identity, affiliation, availability, essential measurements, explicit altitude reference, current execution and a modest original drone illustration labelled as such. New demos use the STING reference profile with 155 km/h horizontal cruise; the expandable profile separates published figures from simulated measurements. Inter is bundled locally under its included SIL OFL license. Measurements/sources, observed history and exact technical values expand on demand. Ordinary time uses 24-hour SGT, with observation/simulation time distinct from wall time, observation lag and delivery freshness. Original UTC values and precision remain copyable.

Activity retains bounded immutable receipts, per-member outcomes and current executions, including cancellation. Its optional coordinate entry uses the same direct path; neither Activity nor coordinate review is required for map commands. Connection, uncertainty and failure notices appear in a compact header attention menu, with an accessible live announcement, even when contextual panes are closed. Routine movement counts stay in Fleet/Details/Activity. The full-width operational strip, shell footer and map footer are removed. Keyboard help lives at the bottom of the navigation rail; map filtering is visible in its toolbar and unlocated counts are in Map layers. Current progress comes only from committed frames; an HTTP receipt ahead of the frame is labelled awaiting world synchronization.

One shared runtime owns transport, control renewal, selection/history and immutable pending requests. It persists full content before sending and reconciles uncertain replies with the original identity, order and deadline, including after reload. Credentials stay in separate private session storage and never enter world frames or recorded evidence. Navigation and lease expiry do not cancel accepted execution.

## Verification and contracts

Current generated contracts are in `contracts/sentinel/v1.4/`; archived versions retain their original meaning. Run `npm run contracts:check`, `npm run typecheck`, `npm run lint`, `npm run format:check`, `npm test -- --maxWorkers=2`, `npm run build`, `npm run build:test`, then `npx playwright test --trace off`. Browser tests use isolated local servers and include actual long pauses and lease expiry. Keep traces off for authority workflows. Current evidence is under `docs/compact-demo/`; historical RTS, M1.2 and Fleet/Details reviews remain unchanged. See [the current review](../docs/compact-demo/REVIEW.md) and [decisions](../docs/compact-demo/DECISIONS.md).

M1.3 remains deferred. The older foundation notes below describe retained architecture; this section supersedes their earlier command gestures, wording, control-entry and framing rules.

# Sentinel v3 workbench â€” entity browsing and observed trails

This package provides the approved compact shell, one shared backend runtime, Tactical MapLibre and Cesium views, Tracks browsing, a shared Fleet sidebar, selection-following Details, pinned entity inspectors and bounded recorded movement trails. Local lifecycle and movement commands are implemented; encounter resolution, operational analytics, external Phase 5 integration and replay playback remain deferred. See the [Phase 3B review](../docs/phase3b/REVIEW.md), [current implementation plan](../docs/IMPLEMENTATION_PLAN.md) and [map account/configuration handoff](../docs/MAP_SERVICES_SETUP.md).
...
Start the backend separately using [backend instructions](../backend/README.md). The Vite server proxies `/api` and WebSockets to `http://127.0.0.1:8000`; `SENTINEL_API_TARGET` overrides that local development/preview target. A deployed server would need the same-origin proxy configured separately. Opening the shell alone does not create a backend connection. For a running local demo choose New demo. Developer fixtures remain under Load mission → Developer fixtures with short display aliases; their internal identities and original records are unchanged. Backend loss retains a visibly stale complete frame until a fresh snapshot succeeds. Errors and retry are available beside the breadcrumb and in its menu.
...
| --- | --- |
| `src/app/App.tsx` | Compact identity/mission header, module Activity Bar, shared Views/Fleet sidebar, operational attention, menus, keyboard help and status bar. Sidebar/dialog state is UI-only. |
...
| `src/features/workspace/viewRegistry.ts` | Registered view kinds, one reusable Details and independently identified Tactical/pinned-inspector instances. No fake mission or entity IDs. |
...
| `src/state/workspaceStore.ts` | Private Zustand store of metadata derived from that model: open views, location, selected tab in each pane, active main-window view, sidebar mode/visibility and revision. It contains no second layout tree. Only the bridge writes it. |
...
| `src/features/workspace/PaneHost.tsx` | Maps, entity/Fleet-related workbench panes, Movement and labelled placeholders, with mount/visibility/resize/disposal callbacks. DOM observation and animation frames are released on unmount. |
...
Renderer resources belong in the mounted view's runtime, outside Zustand and layout JSON. `PaneLifecycleEvent` reports initial visibility, later visibility changes, coalesced size changes (including zero sizes), mount and disposal. The map observes its actual host size, reapplies the latest complete scene after style loading and suspends retained hidden renderers and releases closed renderers through the existing bounded renderer owner. Camera bookmarks carry only geographic intent and a mission tag. The session runtime survives pane open/close/remount; integration with operational pop-outs remains deferred.
...
Browser tests use installed Microsoft Edge, start a fixture backend on 8011 and production previews on 5181/5182, and stop them afterwards. The Python environment must already be installed. The test recording database is retained in ignored `.cache/phase2-browser.sqlite3`; checks compare relative sequence changes or deliberately align the Tactical fixture stage. No browser download is needed on the tested Windows host. The probes run against built bundles and real MapLibre worker/source data. Tests write current evidence under `../docs/rts-refinement/evidence`; earlier review evidence is retained. Traces are disabled because authority headers are private. Map inspection/provider injection probes exist only in the verification build, with a separate normal-production test confirming their absence.
