...
const suffix = process.env.SENTINEL_TEST_BUILD_SUFFIX ?? '-d4-refinement';
const output = resolve('../docs/d4-refinement/regressions');
...
try {
  ...
  const db = resolve(`.cache/d4-refinement-browser-${process.pid}.sqlite3`);
  ownedDatabase = db;
  start(
  ...
  // End unfinished test demos only after the writer is known to have exited.
  if (ownedDatabase && stopped[0]) {
    const cleanup = spawnSync(
      resolve('../backend/.venv/Scripts/python.exe'),
      [resolve('../docs/d4-refinement/cleanup_runtime.py'), ownedDatabase],
