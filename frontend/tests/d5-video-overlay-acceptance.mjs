...
await withD5Runtime(
  ...
  async ({ frontend, output }) => {
    ...
    try {
      ...
      await shot('01-ready');
      expect((await inspect()).videoOverlay.enabled).toBe(true);
      await page
      ...
      await expect(cockpit().locator('.cockpit-notice')).toContainText(
        'VIEW UNAVAILABLE',
      );
      ...
      expect(
        await cockpit()
          .locator('canvas')
          .first()
          .evaluate((e) => globalThis.getComputedStyle(e).filter),
      ...
      await expect(cockpit().locator('.cockpit-notice')).toHaveCount(0);
      expect(
        await cockpit()
          .locator('canvas')
          .first()
