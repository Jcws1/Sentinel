...
await withD5Runtime(
  {
    tag: 'cockpit-refinement-production',
    frontendPort: 5345,
    backendPort: 8145,
    previewDir: 'dist-d5-cockpit-refinement',
    viteConfig: '.cache/d5-cockpit-refinement-preview.mjs',
  },
  async ({ frontend, output }) => {
    ...
    try {
      ...
      await expect(page.locator('.cockpit-notice')).toContainText('DEMO ENDED');
      await expect(page.locator('.cockpit-pane canvas')).toBeVisible();
      report.cases.push(
