...
const output = resolve('../docs/d4-refinement/provider-review');
mkdirSync(output, { recursive: true });
const database = resolve(
  `.cache/d4-refinement-browser-provider-${process.pid}.sqlite3`,
);
const children = [],
  logs = [],
  report = {
...
try {
  await free(8096);
  await free(5296);
  const providerBuild = resolve(
    `.cache/d4-refinement-provider-build-${process.pid}`,
  );
  ...
  if (report.servicesStopped) {
    const cleanup = spawnSync(
      resolve('../backend/.venv/Scripts/python.exe'),
      [resolve('../docs/d4-refinement/cleanup_runtime.py'), database],
