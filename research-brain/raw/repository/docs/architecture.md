...
Interactive authority refreshes share one in-flight promise and one coalesced
follow-up read. A mission/revision change during a read cannot drop the requested
fresh status until the next periodic poll. Callers await the current read, while
generation/disposal checks reject late old-mission replies; lifecycle controls
still require synchronized authoritative state before dispatch.
...
The earlier D7 performance follow-up preserved this architecture. Live and nominal
...
review cache or changed recording representation in that earlier pass. See the
[diagnosis and compatibility boundary](performance-closure/DIAGNOSIS.md).

Milestone 1 adds a bounded saved-revision analysis cache and an explicitly
versioned lossless internal recording envelope. Complete nominal geometry/script
analysis runs in cooperative batches using private canonical scenario content.
It yields between complete nominal ticks on the owner loop; no CPU worker
competes with durable live ticks for the interpreter. Every review refreshes
admission and time after analysis. The cache holds at most eight entries / 2 MiB,
keyed by exact revision/content, analysis rules and applicable profile constants.
Run still performs its authoritative admission and instantiation checks.

SQLite schema 5 permits historical TEXT and a checksummed zlib envelope in frame
and interactive-checkpoint columns. Only new compressible writes use the envelope;
the first such write changes the schema marker inside the same transaction.
Strict historical world readers run after bounded exact decoding. Event journals,
receipts, canonical hashes, cadence, FULL durability and commit-before-publication
remain unchanged. Old schema-4 binaries reject schema 5; opening an old database
with the new reader does not rewrite its historical rows. See
[recording and validation compatibility](d7-details-closure/COMPATIBILITY.md).

Details uses bundled static reference images keyed only by explicit unit-profile
identity. Unsupported or unreliable historical identities show a fallback. These
assets never enter world frames or recordings. Selection, pinned inspector,
telemetry, cameras and simulated Video retain their existing owners.
